      ...
      challenges: {
        store: async (token, challengeData) => {
          try {
            ...
            db.collection("cap_challenges")
              ...
              .catch((err) => logger.error("Failed to create TTL index for cap_challenges", err))
        ...
        read: async (token) => {
          try {
            ...
            return doc ? { challenge: doc.data, expires: doc.expiresAt.getTime() } : null
          } catch (error) {
        ...
        store: async (tokenKey, expires) => {
          try {
            ...
            db.collection("cap_tokens")
              .createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 })
              .catch((err) => logger.error("Failed to create TTL index for cap_tokens", err))
 ...
 */
export async function verifyCaptchaWithDb(token: string, db: Db): Promise<boolean> {
  if (!token) {
