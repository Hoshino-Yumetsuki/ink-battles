...
const app = new Elysia({ aot: false })
  .post('/api/analysis', async ({ body, set }) => {
    try {
      ...
      try {
        ...
        type StrictGenOptions = Parameters<typeof generateText>[0]
        type LooseGenOptions = Omit<StrictGenOptions, 'baseURL'> & {
          baseURL?: string | URL
        }

        const genOptions: LooseGenOptions = {
          apiKey: apiConfig.apiKey as string,
          ...requestConfig,
          messages,
          ...(apiConfig.baseUrl && { baseURL: apiConfig.baseUrl })
        }

        const { text } = await generateText(genOptions as StrictGenOptions)
