...
export function serializeCookie(name: string, value: string, attributes: string[] = []) {
...
export function setCookie(
  ...
  response.headers.append("set-cookie", serializeCookie(name, value, attributes))
}

export function deleteCookie(response: Response, name: string) {
  response.headers.append("set-cookie", serializeCookie(name, "", ["Path=/", "Max-Age=0"]))
}
