...
import { getAccessTokenExpiresIn, getRefreshTokenExpiresIn, signToken } from "@/utils/jwt"
import { z } from "zod"
...
const registerSchema = z.object({
  username: z.string().min(3, "用户名长度至少3个字符").max(20, "用户名长度最多20个字符").trim(),
...
export const POST = withDatabase(async (req: Request, db) => {
  try {
    ...
    const { username: rawUsername, password, email, code, captchaToken } = result.data
    ...
    const encKey = await deriveEncryptionKey(password, insertResult.insertedId.toString())
