...
import { Upload, Sparkles, Loader2, User as UserIcon, LogIn, FileText, Clock } from 'lucide-react'
...
export default function ContentInputCard({
  ...
  const displayPercentage = usageInfo && usageInfo.limit > 0
      ...
      <CardHeader className="pb-2">
        ...
        <div className="mt-4 bg-gray-50/80 dark:bg-zinc-800/50 rounded-xl p-4 border border-gray-100 dark:border-zinc-700">
             ...
             <div className="space-y-3">
               <div className="flex justify-between items-end">
                 <span className="text-sm font-medium text-gray-700 dark:text-gray-300">当前使用额度</span>
               ...
               <div className="flex gap-2 text-xs">
                 <div className="flex-1 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-700 rounded-lg p-2.5 flex justify-between items-center shadow-sm">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white ml-1">{displayUsed} 次</span>
                 </div>
                 <div className="flex-1 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-700 rounded-lg p-2.5 flex justify-between items-center shadow-sm">
                    <span className="text-muted-foreground whitespace-nowrap">每日限额</span>
                    <span className="font-medium text-gray-900 dark:text-white ml-1">{displayLimit} 次</span>
                 </div>
               ...
               <div className="flex justify-between items-center pt-1">
                 ...
                 {!usageInfo?.isLoggedIn ? (
                    <div className="flex gap-3 text-sm font-medium ml-auto">
                      <Link href="/login" className="text-blue-600 hover:text-blue-700 hover:underline">登录</Link>
                      <Link href="/register" className="text-blue-600 hover:text-blue-700 hover:underline">注册</Link>
                    </div>
                 ) : (
        ...
        <div className="mt-6 flex gap-3 items-center">
             <div className="relative">
                ...
                />
                <Button variant="outline" className="gap-2 bg-white dark:bg-zinc-900 hover:bg-gray-50 dark:hover:bg-zinc-800 shadow-sm border-gray-200 dark:border-zinc-700 text-gray-700 dark:text-gray-300">
         ...
         <div className="h-full flex flex-col relative" style={{ minHeight: '300px' }}>
             {analysisType === 'file' && file ? (
                <div className="flex-1 border-2 border-dashed border-gray-200 dark:border-zinc-700 rounded-lg flex flex-col items-center justify-center p-8 bg-gray-50/50 dark:bg-zinc-800/30">
                   ...
                   <Button variant="outline" size="sm" onClick={() => { setFileAction(null); setAnalysisTypeAction('text'); }}>
                      移除文件
                   </Button>
                </div>
             ) : (
               <Textarea
                ...
                value={content}
                onChange={(e) => {
                    if (e.target.value.length <= SINGLE_WORD_LIMIT) {
      ...
      <CardFooter className="pt-2 pb-6 border-t border-gray-100 dark:border-zinc-800/50">
        <div className="w-full flex justify-between items-center">
             <span className="text-sm text-muted-foreground">
               字数统计: {inputWordCount.toLocaleString()} / {SINGLE_WORD_LIMIT.toLocaleString()} 字
             </span>
             <Button
